...
function make_ramdisk_image()
{
	...
	info "**************************"
	cd $OUT_DIR
    cp ramdisk.img $ROOT_DIR/kernel
    cd $ROOT_DIR/kernel
    ./mkbootimg --kernel ./arch/arm/boot/uImage --ramdisk ./ramdisk.img --second ./arch/arm/boot/dts/amlogic/meson8_skt.dtb --output ./m8boot.img
}
